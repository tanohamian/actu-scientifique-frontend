  ...
  useEffect(() => {
    if (!authLoading) {
      if (isLoggedIn) {
        ...
        setLiveEvents([]);
        setUpcomingEvents([]);
        setFormattedCalendarEvents([]);
      }
    }
  }, [isLoggedIn, authLoading, loadEvents]);
              ...
              <div className="p-6 bg-gray-100 min-h-[150px] space-y-4">
                {liveEvents.length > 0 ? (
                  liveEvents.map((event) => (
                    <div
                      key={event.id}
                      className="bg-white rounded-xl p-4 shadow-lg border-l-4 border-orange-500"
                    >
              ...
              <div className="p-4 bg-gray-100 space-y-3 min-h-[250px]">
                {upcomingEvents.length > 0 ? (
                  upcomingEvents.map((event) => (
                    <div
                      key={event.id}
                      className="bg-white rounded-xl p-4 shadow-md border-l-4 border-blue-500 hover:scale-[1.02] transition-transform"
                    >
