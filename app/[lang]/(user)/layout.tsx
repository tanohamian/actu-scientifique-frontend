...
import { useTranslations } from 'next-intl';import { FetchArticles } from '../actions/ArticleManager';
       ...
       {(pathname === "/" || pathname === "/subscription") && (
        ...
        <div className='flex flex-col lg:flex-row items-center justify-between lg:justify-around px-4 py-4 gap-4'>
          ...
          <div className='flex flex-row items-center gap-4 w-full lg:w-auto justify-between lg:justify-end'>
            ...
            {isLoginOpen && (
              <LoginRegisterComponent type='login' title={authT('login')} inputs={inputs} onClose={() => setIsLoginOpen(false)}
                onSubmit={() => { setIsLoginOpen(false); setIsRegisterOpen(true) }} />
            )}
            {isRegisterOpen && (
              <LoginRegisterComponent type='register' title={authT('register')} inputs={inputsRegister} onClose={() => setIsRegisterOpen(false)}
      ...
      <footer className='w-full mt-auto border-white/20'>
        <div className='flex flex-col lg:flex-row justify-around py-8 px-4 gap-8 lg:gap-12'>
          <div className='flex flex-col'>
            <h2 className='text-white text-2xl font-bold mb-4'>{bottomT('contact')}</h2>
            <div className={footerElement + ' mb-3'}>
              <IconComponent name='Localisation' className={`text-white w-6 h-6 shrink-0`} />
              <span className='text-white ml-3 text-sm lg:text-base'>Abidjan, Lagoona City, Immeuble 53, Porte 08</span>
            </div>
            <div className={footerElement}>
              <IconComponent name='Phone' className={`text-white w-6 h-6 shrink-0`} />
          ...
          <div className='flex flex-col'>
            ...
            <a href="/about" className='text-white hover:underline mb-2 text-sm lg:text-base'>{bottomT("mission")}</a>
            <a href="/charte-editoriale.pdf" target="_blank" rel="noopener noreferrer" className='text-white hover:underline text-sm lg:text-base'>{bottomT('charter')}</a>
          </div>
          <div className='flex flex-col'>
            <h2 className='text-white text-2xl font-bold mb-4'>{bottomT("partners")}</h2>
