...
import { Article, DbMedia, IUpdateArticle } from "../interfaces";
...
export type ElementType = DbMedia | Article | TableData | IUpdateArticle;
...
export default function DataTable({
  ...
  return (
    <div
      className={
        "bg-[#50789B] p-5 rounded-lg mt-5 shadow-xl font-sans " + isMedia
          ? "w-full bg-[#50789B] p-5 rounded-lg mt-5 shadow-xl font-sans "
          : "w-200"
      }
    >
          ...
          >
            {columnHeaders.map((header) => {
              ...
              if (header.key === "status") {
                content = (
                  <span
                    className={getStatusClasses((item as TableData).status)}
                  >
                    {(item as TableData).status}
                  </span>
                );
              }

              if (header.key === "rubrique" && (item as Article).rubrique) {
                content = translateRubrique(
                  (item as Article).rubrique as string,
                );
              }
                  ...
                  handleDeleteEvent && handleEditEvent ? (
                    <div className="flex gap-2">
                      <button
                        ...
                        onClick={
                          handleEditEvent
                            ? () => handleEditEvent(item as TableData)
                            : undefined
                        }
                      >
                        <Pencil size={20} />
                      ...
                      <button
                        ...
                        onClick={
                          handleDeleteEvent
                            ? () => handleDeleteEvent(item as TableData)
                            : undefined
                        }
                      >
                        <Trash2 size={20} />
