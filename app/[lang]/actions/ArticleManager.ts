...
import { Article, DbArticle, IUpdateArticle } from "../interfaces";
...
export async function AddArticle(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw Error(
        `Échec de l'upload du média : ${response.status} ${response.statusText}`,
      );
    }
...
export async function UpdateArticle(articleId: string, data: FormData) {
  const lang = await getLocale();
  const baseUrl = env.getApiUrl(lang as LANG);
  const authToken = (await cookies()).get("authToken")?.value;
  if (!authToken) {
